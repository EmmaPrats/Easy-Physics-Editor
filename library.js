...
{
    var circle = new Vector (this.velocity.x, this.velocity.y);
    if (circle.getMagnitude() == 0)
    {
        circle.x = 1;
        circle.y = 0;
    }
    ...
    circle.mult (0.2 * this.radius);
    
    var displacement = new Vector (2*this.radius * Math.cos(this.wanderAngle), 2*this.radius * Math.sin(this.wanderAngle));
    var a = getRandomBetween (-0.1, 0.1);
    this.wanderAngle += getRandomBetween (-0.5, 0.5);//-Math.PI/2, Math.PI/2);
    
    var wanderForce = Vector.add (circle, displacement);
